...
@mock.patch('iib.workers.tasks.build._push_image')
@mock.patch('iib.workers.tasks.build._build_image')
@mock.patch('iib.workers.tasks.build._deprecate_bundles')
@mock.patch('iib.workers.tasks.build._get_external_arch_pull_spec')
@mock.patch('iib.workers.tasks.build._get_bundles_from_deprecation_list')
@mock.patch('iib.workers.tasks.build._add_bundles_missing_in_source')
@mock.patch('iib.workers.tasks.build._create_intermediate_index_image')
@mock.patch('iib.workers.tasks.build._get_present_bundles')
@mock.patch('iib.workers.tasks.build.set_request_state')
@mock.patch('iib.workers.tasks.build._update_index_image_build_state')
@mock.patch('iib.workers.tasks.build._prepare_request_for_build')
...
@mock.patch('iib.workers.tasks.build.set_request_state')
@mock.patch('iib.workers.tasks.build.run_cmd')
def test_create_intermediate_index_image(
    mock_run_cmd, mock_srs, mock_aolti, mock_bi, mock_pi, mock_capml, mock_srt
):
    build._create_intermediate_index_image(
        'binary-image:4.6', 'from-index:4.5', '4.6', 'some_dir', 1, 'amd64'
    )
    cmd = [
        'opm',
        'index',
        ...
        'from-index:4.5',
        '--operators',
        'dummy-nonexistant-operator',
    ]
    mock_run_cmd.assert_called_once_with(
        cmd, {'cwd': 'some_dir'}, exc_msg='Failed to generate new index image Dockerfile'
    )
    mock_aolti.assert_called_once_with('4.6', 'some_dir', 'index.Dockerfile')
    mock_bi.assert_called_once_with('some_dir', 'index.Dockerfile', 1, 'amd64')
    mock_pi.assert_called_once_with(1, 'amd64')
    mock_capml.assert_called_once_with(1, ['amd64'])
...
def test_add_bundles_missing_in_source(
    mock_srs, mock_geaps, mock_oia, mock_aolti, mock_bi, mock_pi, mock_capml
):
    source_bundles = [
    ...
    mock_geaps.return_value = 'localhost/index-image:1'
    missing_bundles = build._add_bundles_missing_in_source(
        source_bundles, target_bundles, 'some_dir', 'binary-image:4.5', 1, 'amd64', '4.6'
    )
    ...
    mock_oia.assert_called_once_with(
        'some_dir',
        ['quay.io/bundle3:123456', 'quay.io/bundle4:123456'],
        'binary-image:4.5',
        'localhost/index-image:1',
    )
    mock_aolti.assert_called_once()
...
def test_add_bundles_missing_in_source_none_missing(
    ...
    assert missing_bundles == []
    mock_srs.assert_called_once()
    assert mock_geaps.call_count == 0
    assert mock_oia.call_count == 0
    assert mock_aolti.call_count == 0
    assert mock_bi.call_count == 0
    assert mock_pi.call_count == 0
    assert mock_capml.call_count == 0
...
def test_get_bundles_from_deprecation_list(mock_gri):
    ...
    deprecate_bundles = build._get_bundles_from_deprecation_list(present_bundles, deprecation_list)
        ...
        (
            [
                (
                    ...
                    {'Labels': {'com.redhat.openshift.versions': '=v4.6'}},
                ),
                (
                    {
                        'packageName': 'bundle2',
                    ...
                    {'Labels': {'com.redhat.openshift.versions': '=v4.5'}},
                ),
                (
                    {
                        'packageName': 'bundle3',
        ...
        (
            [
                (
                    ...
                    {'Labels': {'com.redhat.openshift.versions': 'v4.4-v4.6'}},
                ),
                (
                    {
                        'packageName': 'bundle2',
                    ...
                    {'Labels': {'com.redhat.openshift.versions': 'v4.7-v4.8'}},
                ),
                (
                    {
                        'packageName': 'bundle3',
                    ...
                    {'Labels': {'com.redhat.openshift.versions': 'v4.5-v4.7'}},
                ),
            ],
            'v4.5',
            [
        ...
        (
            [
                (
                    ...
                    {'Labels': {'com.redhat.openshift.versions': 'v4.5,v4.6'}},
                ),
                (
                    {
                        'packageName': 'bundle2',
                    ...
                    {'Labels': {'com.redhat.openshift.versions': 'v4.4,v4.5'}},
                ),
                (
                    {
                        'packageName': 'bundle3',
                    ...
                    {'Labels': {'com.redhat.openshift.versions': 'v4.7,v4.8,v4.9'}},
                ),
            ],
            'v4.6',
            [
        ...
        (
            [
                ...
                (
                    ...
                    {'Labels': {'com.redhat.openshift.versions': 'v4.7'}},
                ),
            ],
            'v4.6',
            [
...
def test_get_deprecated_bundles_by_ocp_version(
    ...
    deprecate_bundles = build._get_deprecated_bundles_by_ocp_version(
...
@mock.patch('iib.workers.tasks.build.skopeo_inspect')
@mock.patch('iib.workers.tasks.build._get_deprecated_bundles_by_ocp_version')
def test_check_operator_heads_for_deprecation(mock_gdbbov, mock_si):
    bundles = [
        {
            'packageName': 'bundle1',
            'version': '1.0',
    ...
    build._check_operator_heads_for_deprecation(bundles, 'v4.6')
