...
def test_handle_containerized_add_request_failure(
    ...
    # Mock git repo preparation to return something unpacked
    ...
    # Setup a failure deeper in the process (inside the try block)
    mock_git_commit.side_effect = Exception("Git error")

    with pytest.raises(IIBError, match="Failed to add FBC fragment: Git error"):
        build_containerized_add.handle_containerized_add_request(
