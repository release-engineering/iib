...
def test_handle_regenerate_bundle_request(
    ...
    mock_aob.assert_called_once_with(
        ...
        request_id,
        organization='acme',
        pinned_by_iib=pinned_by_iib_bool,
...
def test_adjust_operator_bundle_unordered(
    ...
    def get_resolved_image(image):
        return {
            'quay.io/operator/image:v1': 'quay.io/operator/image@sha256:749327',
...
def test_adjust_operator_bundle_ordered(
    ...
    def get_resolved_image(image):
        return {
            'quay.io/operator/image:v1': 'quay.io/operator/image@sha256:749325',
...
def test_adjust_operator_bundle_related_images_with_env_var(
                      ...
                      template:
                        spec:
                          ...
                          - name: image-annotation
                            image: {registry}/{operator}{image}{ref}
                            env:
                            - name: RELATED_IMAGE_SOMETHING
                              value: {registry}/{operator}{image}{related_ref}
              relatedImages:
              - name: {related_name}
                image: {registry}/{operator}{image}{related_ref}
            """
    ...
    assert csv.read_text('utf-8') == csv_template.format(
        registry='quay.io',
        operator='namespace/reponame',
        image='-rhel-8-final',
        ref='@sha256:749325',
...
@mock.patch('iib.workers.tasks.build_regenerate_bundle._adjust_csv_annotations')
@mock.patch('iib.workers.tasks.build_regenerate_bundle.get_image_labels')
@mock.patch('iib.workers.tasks.build_regenerate_bundle._is_bundle_image')
@mock.patch('iib.workers.tasks.build_regenerate_bundle.get_related_bundle_images')
@mock.patch('iib.workers.tasks.build_regenerate_bundle.write_related_bundles_file')
def test_adjust_operator_bundle_perform_bundle_replacements(
    mock_wrbf, mock_grbi, mock_ibi, mock_gil, mock_aca, mock_gri, mock_apns, mock_gpa, tmpdir
):
    annotations = {
        'marketplace.company.io/remote-workflow': (
            'https://marketplace.company.com/en-us/operators/{package_name}/pricing'
    ...
    def ibi_func(image_name):
        print('foo')
        return image_name == 'quay.io/operator/image@sha256:654321'

    mock_ibi.side_effect = ibi_func
    manifests_dir = tmpdir.mkdir('manifests')
    metadata_dir = tmpdir.mkdir('metadata')
    csv1 = manifests_dir.join('2.clusterserviceversion.yaml')
    csv2 = manifests_dir.join('3.clusterserviceversion.yaml')
