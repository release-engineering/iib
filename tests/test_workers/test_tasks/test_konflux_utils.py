...
def test_find_pipelinerun_empty_result(mock_get_worker_config, mock_get_client):
    """Test pipelinerun search with empty results raises IIBError after retries."""
    # Setup
    mock_client = Mock()
    mock_get_client.return_value = mock_client
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('pipelineResults', 'quay.io/namespace/image:tag', 'Older Tekton format with pipelineResults'),
    ],
)
...
def test_get_pipelinerun_image_url_error_cases(run, description):
    ...
    # Test & Verify
    with pytest.raises(IIBError, match='IMAGE_URL not found in pipelinerun test-pipelinerun results'):
