...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "Invalid pullspec format: 'registry.example.com/namespace/iib-pub-pending'. Missing tag (':') delimiter.",
        ),
        (
            "registry.example.com/namespace/image:",
            "Invalid pullspec format: 'registry.example.com/namespace/image:'. Could not parse name:tag structure.",
        ),
        (
...
def test_refresh_indexdb_cache_for_image_propagates_exception(mock_refresh_cache):
    """Test that refresh_indexdb_cache_for_image propagates exceptions from refresh_indexdb_cache."""
    from iib.workers.tasks.oras_utils import refresh_indexdb_cache_for_image
