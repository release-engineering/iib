...
def test_set_registry_token(
    ...
    if template_exists:
        mock_open.assert_has_calls(
            (
                # _load_docker_config_auths reads the existing config to decide token scope
                mock.call('/home/iib-worker/.docker/config.json', 'r'),
...
@mock.patch('iib.workers.tasks.utils.os.path.exists')
@mock.patch('iib.workers.tasks.utils.open')
def test_load_docker_config_auths(mock_open, mock_exists):
    mock_exists.return_value = True
    mock_open.side_effect = mock.mock_open(
        read_data=json.dumps({'auths': {'quay.io/ns': {'auth': 'token'}}})
    )

    assert utils._load_docker_config_auths() == {'quay.io/ns': {'auth': 'token'}}


@mock.patch('iib.workers.tasks.utils.os.path.exists')
@mock.patch('iib.workers.tasks.utils.open')
def test_load_docker_config_auths_raises_on_unreadable_config(mock_open, mock_exists):
    # A config that exists but cannot be parsed must fail loudly rather than be treated as
    # "no auths", which would broaden overwrite-token stamping.
    mock_exists.return_value = True
    mock_open.side_effect = mock.mock_open(read_data='not valid json')

    with pytest.raises(IIBError, match='Failed to read Docker config auths'):
        utils._load_docker_config_auths()
...
@pytest.mark.parametrize(
    'from_index, from_index_arches, bundles, binary_image,'
    'expected_bundle_mapping, distribution_scope, resolved_distribution_scope, binary_image_config',
    (
        (
            'some-index:latest',
        ...
        (None, set(), None, 'scratch', {}, None, 'prod', {'prod': {'v4.5': 'scratch'}}),
        (
            'some-index:latest',
            {'amd64'},
            None,
...
@mock.patch('iib.workers.tasks.utils.get_image_label')
@mock.patch('iib.workers.tasks.build.update_request')
def test_prepare_request_for_build_binaryless(
    mock_ur,
    mock_gil,
    mock_gia,
    mock_gri,
...
@pytest.mark.parametrize(
    'from_index, binary_image, binary_image_config,error',
    (
        (
            'some-index:latest',
            'binary-image:latest',
            {'prod': {'v4.5': 'scratch'}},
            'The index image must be built as a binaryless for v4.5',
        ),
        (
            'some-index:latest',
            'scratch',
            {'prod': {'v4.5': 'binary-image:latest'}},
            'The index image should not be binaryless for v4.5',
        ),
    ),
)
@mock.patch('iib.workers.tasks.utils.set_request_state')
...
@mock.patch('iib.workers.tasks.utils.get_resolved_image')
@mock.patch('iib.workers.tasks.utils.get_image_arches')
def test_prepare_request_for_build_binaryless_invalid_conditions(
    mock_gia, mock_gri, mock_giii, mock_srs, from_index, binary_image, binary_image_config, error
) -> None:
    mock_giii.return_value = {
        'resolved_from_index': None,
        'ocp_version': 'v4.5',
        'arches': set(),
        'resolved_distribution_scope': 'prod',
    }
    mock_gri.return_value = 'scratch'
    mock_gia.return_value = {'amd64'}
...
def test_get_binary_image_config_no_config_val():
    ...
    b_img_cfg = {'prod': {'v4.6': 'binary_image'}}
    rc = utils.RequestConfig(distribution_scope="prod", binary_image_config=b_img_cfg)
...
@mock.patch('iib.workers.tasks.oras_utils.get_image_digest')
def test_get_indexdb_artifact_pullspec_invalid(mock_digest):
...
@mock.patch('iib.workers.tasks.oras_utils.get_worker_config')
@mock.patch('iib.workers.tasks.oras_utils.get_image_digest')
def test_get_imagestream_artifact_pullspec(mock_digest, mock_gwc, from_index, digest):
    """Test constructing ImageStream artifact pullspecs."""
    mock_gwc.return_value = {
        'iib_index_db_imagestream_registry': 'test-imagestream-registry',
        'iib_index_db_artifact_template': '{registry}/index-db:{tag}',
        'iib_index_db_artifact_tag_template': 'idb-{digest}',
    }
    mock_digest.return_value = f'sha256:{digest}'
    ...
    assert result == f'test-imagestream-registry/index-db:idb-{digest}'


@mock.patch('iib.workers.tasks.oras_utils.get_image_digest')
def test_get_imagestream_artifact_pullspec_invalid(mock_digest):
    """Test get_imagestream_artifact_pullspec propagates digest resolution failures."""
    mock_digest.side_effect = IIBError('Failed to inspect image')
