...
class TestGetKafkaProducer:
    ...
    def test_get_kafka_producer_success(self, mock_kp_class, app):
        ...
        # security_protocol and sasl_mechanism are hardcoded, not configurable
    ...
    @mock.patch('iib.web.kafka_producer.KafkaProducer')
    def test_get_kafka_producer_ssl_cafile_default(self, mock_kp_class, app):
        ...
        call_kwargs = mock_kp_class.call_args[1]
        assert call_kwargs['ssl_cafile'] == '/etc/pki/tls/certs/ca-bundle.crt'

    @mock.patch('iib.web.kafka_producer.KafkaProducer')
    def test_get_kafka_producer_ssl_cafile_custom(self, mock_kp_class, app):
        ...
        call_kwargs = mock_kp_class.call_args[1]
        assert call_kwargs['ssl_cafile'] == '/custom/ca-bundle.crt'

    @mock.patch('iib.web.kafka_producer.KafkaProducer')
    def test_get_kafka_producer_ssl_cafile_omitted_when_falsy(self, mock_kp_class, app):
        """Verify ssl_cafile is omitted from kwargs when explicitly unset."""
        app.config['IIB_KAFKA_BROKERS'] = ['broker:9096']
        app.config['IIB_KAFKA_USERNAME'] = 'u'
        app.config['IIB_KAFKA_PASSWORD'] = 'p'
        ...
        call_kwargs = mock_kp_class.call_args[1]
        assert 'ssl_cafile' not in call_kwargs

    def test_get_kafka_producer_missing_username(self, app):
        """Verify None is returned and no producer is created when username is missing."""
        app.config['IIB_KAFKA_BROKERS'] = ['broker:9096']
        app.config['IIB_KAFKA_PASSWORD'] = 'pass'

        producer = kafka_producer.get_kafka_producer()

        assert producer is None

    def test_get_kafka_producer_missing_password(self, app):
        """Verify None is returned and no producer is created when password is missing."""
        app.config['IIB_KAFKA_BROKERS'] = ['broker:9096']
        app.config['IIB_KAFKA_USERNAME'] = 'user'

        producer = kafka_producer.get_kafka_producer()

        assert producer is None

    def test_get_kafka_producer_missing_credentials(self, app):
        """Verify None is returned when neither username nor password is configured."""
        app.config['IIB_KAFKA_BROKERS'] = ['broker:9096']
...
class TestSendKafkaMessage:
    ...
    def test_send_kafka_message_non_kafka_error(self, app):
        """
        Verify non-KafkaError exceptions are also caught and do not raise.

        ``send_kafka_message`` catches ``Exception`` broadly (not just ``KafkaError``)
        since this is a best-effort, fire-and-forget notification path that must never
        crash the caller (e.g. a JSON serialization ``TypeError`` from a bad payload).
        """
        mock_producer = mock.Mock()
        mock_producer.send.side_effect = TypeError('serialization error')
    ...
    def test_send_kafka_message_unexpected_error(self, app):
        """Verify arbitrary unexpected exceptions (not just TypeError) are also swallowed."""
        mock_producer = mock.Mock()
        mock_producer.send.side_effect = RuntimeError('unexpected failure')
