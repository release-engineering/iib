...
* `iib_index_db_oras_auth_secret` - the authentication secret for the private registry hosting the
   index.db artifact, given by the `iib_index_db_artifact_registry`. When both configuration variables
   are set, the ORAS mechanism will use this authentication exclusively for its operation, without
   relying on the current Docker config.json for all registries.
