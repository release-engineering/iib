...
def upgrade():
    op.create_table(
        'add_request_bundle_deprecation',
    ...
    with op.batch_alter_table('add_request_bundle_deprecation', schema=None) as batch_op:
        batch_op.create_index(
            batch_op.f('ix_add_request_bundle_deprecation_bundle_id'), ['bundle_id'], unique=False
        )
        batch_op.create_index(
            batch_op.f('ix_add_request_bundle_deprecation_request_add_id'),
            ['request_add_id'],
            unique=False,
        )
...
def downgrade():
    with op.batch_alter_table('add_request_bundle_deprecation', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('ix_add_request_bundle_deprecation_request_add_id'))
        batch_op.drop_index(batch_op.f('ix_add_request_bundle_deprecation_bundle_id'))

    op.drop_table('add_request_bundle_deprecation')
