...
def get_kafka_producer() -> Optional[Producer]:
    ...
    Returns ``None`` when ``IIB_KAFKA_BROKERS`` or ``IIB_KAFKA_USERNAME`` is not configured,
    or when the ``IIB_KAFKA_PASSWORD`` environment variable is not set, or when the producer
    fails to initialize.
    ...
    password = os.getenv('IIB_KAFKA_PASSWORD')
...
def _on_delivery_callback(err: Any, msg: Any) -> None:
    """
    ...
    if err:
        ...
        logging.getLogger(__name__).error(
            'Failed to deliver message to Kafka topic %s (key=%s): %s',
            msg.topic(),
...
def send_kafka_message(
    ...
    try:
        ...
        producer.produce(
            ...
            headers=headers,
            on_delivery=_on_delivery_callback,
        )
