...
def _build_request_state_change_data(
    request: Request,
) -> Tuple[BaseClassRequestResponse, Dict[str, Any]]:
    """
...
) -> Optional[Tuple[BatchRequestResponseList, Dict[str, Any]]]:
    """
...
def _send_kafka_messages(
    ...
    try:
        ...
        if not producer:
            current_app.logger.debug(
                'No Kafka messages will be sent since no Kafka producer is available'
            )
            return
        ...
        if not build_topic:
            current_app.logger.debug(
                'No Kafka build state change messages will be sent since the configuration '
                '"IIB_KAFKA_BUILD_STATE_TOPIC" is not set'
            )
        else:
            for request in requests:
                current_app.logger.info(
                    'Sending a Kafka state change message for request %d to topic %s',
                    request.id,
                    build_topic,
                )
                content, properties = _build_request_state_change_data(request)
        ...
        if not batch_topic:
            current_app.logger.debug(
                'No Kafka batch state change message will be sent since the configuration '
                '"IIB_KAFKA_BATCH_STATE_TOPIC" is not set'
            )
        else:
            batch_data = _build_batch_state_change_data(batch, new_batch)
            if not batch_data:
                current_app.logger.debug(
                    'No Kafka batch state change message is needed for batch %d', batch.id
                )
            else:
                current_app.logger.info(
                    'Sending a Kafka state change message for batch %d to topic %s',
                    batch.id,
                    batch_topic,
                )
                batch_content, batch_properties = batch_data
                send_kafka_message(producer, batch_topic, batch_content, batch_properties)
    except Exception:
        current_app.logger.exception('Failed to send one or more messages')
