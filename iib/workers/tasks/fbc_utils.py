...
def extract_fbc_fragment(
    ...
    _copy_files_from_image(fbc_fragment, conf['fbc_fragment_catalog_path'], fbc_fragment_base_path)

    # podman cp creates a subdirectory named after the source path basename, e.g.
    # /tmp/iib-**/fbc-fragment-0/configs/example-operator. Match get_catalog_dir().
    fbc_fragment_catalog_dir = os.path.join(
        fbc_fragment_base_path, os.path.basename(conf['fbc_fragment_catalog_path'])
    )
