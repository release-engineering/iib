...
def _load_docker_config_auths() -> Dict[str, Any]:
    ...
    for path in candidate_paths:
        ...
        try:
            ...
            # A config that exists but cannot be read/parsed must not be silently treated as
            # "no auths": that would make every image look uncovered and broaden token
            # stamping. Fail loudly, consistent with the append path in set_registry_token.
            log.error('Failed to read Docker config auths from %s: %s', path, e)
            raise IIBError(f'Failed to read Docker config auths from {path}: {e}')
    return {}
...
def get_images_needing_overwrite_token(from_index: Optional[str], images: List[str]) -> List[str]:
    """
...
def set_registry_token(
    ...
    * ``registry`` when the image has no namespace (for example, ``localhost:5000/myimage:tag``)
    ...
    for image in images:
        ...
        #
        # This namespace fallback is deliberate, not redundant with the repo key above: some
        # pull paths do not honor repo-scoped keys, so the repo key alone is insufficient. The
        # accepted trade-off is that another same-namespace image pulled anonymously or with a
        # different credential would inherit this token; the conditions below (same namespace,
        # no pre-existing path auth, namespace key only) bound that blast radius. Do not
        # "simplify" this away to the repo key only.
