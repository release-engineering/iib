...
def opm_registry_add_fbc(
    ...
    # set_registry_token is a no-op when bundles_needing_token is empty, so a single call
    # covers both the "some bundles need the token" and "none need it" cases.
    with set_registry_token(overwrite_from_index_token, bundles_needing_token, append=True):
        _opm_registry_add(
            base_dir=base_dir,
            index_db=index_db_file,
            bundles=bundles,
...
def opm_registry_add_fbc_fragment(
    ...
    # set_registry_token is a no-op when overwrite_from_index_token is empty.
    with set_registry_token(overwrite_from_index_token, from_index, append=True):
    ...
    # set_registry_token is a no-op when fragments_needing_token is empty, so a single call
    # covers both the "some fragments need the token" and "none need it" cases.
    with set_registry_token(overwrite_from_index_token, fragments_needing_token, append=True):
...
def verify_operators_exists(
    ...
    if (not index_db_path or not os.path.exists(index_db_path)) and from_index:
        ...
        # and to remove those fbc-operations endpoint should be used
        with set_registry_token(overwrite_from_index_token, from_index, append=True):
