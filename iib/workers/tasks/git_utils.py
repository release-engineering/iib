...
def push_configs_to_git(
    ...
    with tempfile.TemporaryDirectory(prefix=f"git-repo-{request_id}-") as local_repo_dir:
        ...
        try:
            clone_gitlab_repo(repo_url, branch, gitlab_token_name, gitlab_token, local_repo_dir)
            ...
            log.info(git_status)
            commit_and_push(request_id, local_repo_dir, repo_url, branch)
        finally:
...
def get_gitlab_url(from_index) -> str:
    ...
    gitlab_url = index_repo_map.get(index_no_tag, None)
    if not gitlab_url:
