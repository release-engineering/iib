...
def get_indexdb_artifact_pullspec(from_index: str) -> str:
    """
    Constructs the full pullspec for index_db artifact.
...
def push_oras_artifact(
    ...
    # Do not allow absolute paths - they are extracted to the same place (full path) which might cause collisions.
...
def verify_indexdb_cache_for_image(index_image_pullspec: str) -> str:
    """
    Verifies the synchronization state of the index database cache for a given
    container image.
