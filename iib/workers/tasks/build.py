...
def _create_intermediate_index_image(
    binary_image,
    from_index,
    ocp_version,
    base_dir,
    ...
    """
    Create index image with the same data as old one, but with new binary image and ocp version.
    ...
    :param str overwrite_from_index_token: the token used for overwriting the input
        ...
        ``overwrite_from_index``. The format of the token must be in the format "user:password".
    """
    set_request_state(request_id, 'in_progress', 'Creating intermediate index image')
    log.info('Creating intermediate index image with a new binary image')
    # we'll generate Dockerfile using a no-op rm command
    cmd = [
        'opm',
        'index',
        'rm',
    ...
    with set_registry_token(overwrite_from_index_token, from_index):
        run_cmd(
            cmd, {'cwd': base_dir}, exc_msg='Failed to generate new index image Dockerfile',
        )
    ...
    log.info('Intermediate image created')
...
def _add_bundles_missing_in_source(
    ...
    missing_bundles = []
    source_bundle_ids = [
        (bundle['packageName'], bundle['version']) for bundle in source_index_bundles
    ]
    target_bundle_ids = [
        (bundle['packageName'], bundle['version'], bundle) for bundle in target_index_bundles
    ]

    for package_name, version, bundle in target_bundle_ids:
        if (package_name, version) not in source_bundle_ids:
    ...
    if missing_bundles != []:
        ...
        _opm_index_add(base_dir, missing_bundle_paths, binary_image, intermediate_image_name)
        _add_ocp_label_to_index(ocp_version, base_dir, 'index.Dockerfile')
        _build_image(base_dir, 'index.Dockerfile', request_id, arch)
        _push_image(request_id, arch)
        _create_and_push_manifest_list(request_id, [arch])
        log.info('Image with missing bundles created')
    else:
        log.info(
            'No bundles present in the target index image are missing from the source index image.'
        )
...
def _get_deprecated_bundles_by_ocp_version(bundles_with_metadata, ocp_version):
    """
    Parse ocp version and determine if the bundles should be deprecated.

    :param list bundles_with_metadata: list of tuples containing bundles and their metadata.
    ...
    deprecate_bundles = []
    for bundle, metadata in bundles_with_metadata:
        bundle_ocp_version = metadata['Labels']['com.redhat.openshift.versions']
        # only one version
...
def _check_operator_heads_for_deprecation(bundles, ocp_version):
    ...
    bundles_with_metadata = []
    for head_bundle in channel_heads:
        skopeo_out = skopeo_inspect('docker://{0}'.format(head_bundle['bundlePath']))
        bundles_with_metadata.append((head_bundle, skopeo_out))

    return _get_deprecated_bundles_by_ocp_version(bundles_with_metadata, ocp_version)
...
def handle_merge_request(
    ...
    with tempfile.TemporaryDirectory(prefix='iib-') as temp_dir:
        ...
        _create_intermediate_index_image(
            binary_image,
            source_from_index,
        ...
        # Deprecating based on label should not be used for now
        # This functionality will be added later, after the metadata format is put into use
        # intermediate_bundles = _get_present_bundles(intermediate_image_name, temp_dir)
        # deprecate_bundles = _check_operator_heads_for_deprecation(
        ...
        #   request_id,
        #   'in_progress',
        #   'Deprecating bundles that don\'t satisfy the ocp version'
        # )
        # _deprecate_bundles(
        #     deprecate_bundles,
        #     temp_dir,
