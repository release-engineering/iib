...
# Subclasses IIBError so existing "except IIBError" handlers still catch it, while letting
# callers distinguish a genuinely absent path from a real extraction failure (registry, OCI
# parsing, layer, or tar error).
class FileNotFoundInImageError(IIBError):
    """A requested path was not present in a container image."""


# Subclasses IIBError for the same reason as FileNotFoundInImageError: callers that only care
# that the pull failed keep working, while callers that must tell "this image was never
# onboarded" apart from a registry outage or an auth failure can catch this specifically.
